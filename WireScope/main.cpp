...
namespace
{
    ...
    void DeleteUiObjects(AppState& s)
    {
        ...
        if (s.titleFont) { DeleteObject(s.titleFont); s.titleFont = nullptr; }
        if (s.metricFont) { DeleteObject(s.metricFont); s.metricFont = nullptr; }
        if (s.smallFont) { DeleteObject(s.smallFont); s.smallFont = nullptr; }
    ...
    void ApplyTheme(AppState& s)
    {
        ...
        ReleaseDC(s.hwnd, dc);
        const auto createFont = [&](int points, int weight) -> HFONT
        {
            const int height = -MulDiv(points, dpiY, 72);
            return CreateFontW(height, 0, 0, 0, weight, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
                OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_DONTCARE,
                s.settings.font_name);
        };
        s.font = createFont(s.settings.font_size, FW_NORMAL);
        s.titleFont = createFont((std::max)(20, s.settings.font_size + 12), FW_BOLD);
        s.metricFont = createFont((std::max)(14, s.settings.font_size + 5), FW_BOLD);
        s.smallFont = createFont((std::max)(8, s.settings.font_size - 1), FW_NORMAL);
        ...
        if (s.adapterCombo) SetWindowTheme(s.adapterCombo, L"DarkMode_CFD", nullptr);
        if (s.presetCombo) SetWindowTheme(s.presetCombo, L"DarkMode_CFD", nullptr);
        if (s.searchEdit) SetWindowTheme(s.searchEdit, L"DarkMode_CFD", nullptr);
    ...
    void DrawOwnerButton(const DRAWITEMSTRUCT* dis, AppState& s)
    {
        ...
        COLORREF fillColor = Mix(s.settings.background_color, RGB(255, 255, 255), pressed ? 0.12 : 0.065);
        COLORREF borderColor = Mix(s.settings.background_color, RGB(255, 255, 255), 0.16);
        COLORREF textColor = disabled ? Mix(s.settings.text_color, s.settings.background_color, 0.58) : s.settings.text_color;

        if (id == IDC_START && !disabled)
        {
            fillColor = pressed ? Mix(s.settings.accent_color, RGB(0, 0, 0), 0.18) : s.settings.accent_color;
            borderColor = Mix(s.settings.accent_color, RGB(255,255,255), 0.28);
            textColor = RGB(255,255,255);
        }
        else if (id == IDC_FILTERS || id == IDC_SETTINGS)
        {
            fillColor = Mix(s.settings.background_color, s.settings.accent_color, pressed ? 0.18 : 0.055);
            borderColor = s.settings.accent_color;
        }
        else if (id == IDC_STOP && !disabled)
        {
            fillColor = Mix(s.settings.background_color, s.settings.accent_color, 0.08);
            borderColor = Mix(s.settings.accent_color, s.settings.background_color, 0.40);
        }

        HBRUSH fill = CreateSolidBrush(fillColor);
        HPEN pen = CreatePen(PS_SOLID, focused ? 2 : 1, borderColor);
    ...
    void LoadAdapters(AppState& s)
    {
        ...
        {
            std::wstring text = (*a.name ? a.name : L"Network adapter");
            text += L"  \u2014  IPv4: ";
            text += (*a.ipv4 ? a.ipv4 : L"\u2014");
    ...
    void DrawRoundedPanel(HDC dc, const RECT& rect, COLORREF fillColor, COLORREF borderColor, int radius = 16)
    {
        HBRUSH fill = CreateSolidBrush(fillColor);
    ...
    std::wstring CurrentAdapterText(AppState& s)
    {
        if (!s.adapterCombo) return L"No adapter selected";
        const int index = static_cast<int>(SendMessageW(s.adapterCombo, CB_GETCURSEL, 0, 0));
        if (index < 0) return L"No adapter selected";
        wchar_t text[512]{};
        SendMessageW(s.adapterCombo, CB_GETLBTEXT, index, reinterpret_cast<LPARAM>(text));
        return *text ? text : L"No adapter selected";
    }

    std::wstring CurrentFilterText(AppState& s)
    {
        WS_FILTER_CONFIG config{};
        { std::scoped_lock guard(s.filterMutex); config = s.filter; }
        std::wstring out;
        if (config.incoming_only) out += L"Incoming";
        if (config.discord_only)
        {
            if (!out.empty()) out += L"  \u2022  ";
            out += L"Discord";
        }
        else if (config.protocol == WS_PROTOCOL_TCP || config.protocol == WS_PROTOCOL_UDP)
        {
            if (!out.empty()) out += L"  \u2022  ";
            out += ProtocolText(config.protocol);
        }
        if (config.port_count > 0)
        {
            if (!out.empty()) out += L"  \u2022  ";
            out += std::to_wstring(config.port_count) + L" port rule";
            if (config.port_count != 1) out += L"s";
        }
        ...
        if (config.hide_noise)
        {
            if (!out.empty()) out += L"  \u2022  ";
            out += L"Noise hidden";
        }
        return out.empty() ? L"No filter" : out;
    }
    ...
    void PaintWindowBackground(AppState& s, HDC dc, const RECT& client)
    {
        ...
        const COLORREF panel = Mix(s.settings.background_color, RGB(255,255,255), 0.035);
        const COLORREF panel2 = Mix(s.settings.background_color, RGB(255,255,255), 0.055);
        const COLORREF border = Mix(s.settings.background_color, RGB(255,255,255), 0.13);
        const COLORREF muted = Mix(s.settings.text_color, s.settings.background_color, 0.46);

        RECT hero{18, 14, client.right - 18, 128};
        DrawRoundedPanel(dc, hero, panel, border, 18);
        RECT adapterCard{18, 140, (client.right * 62) / 100, 210};
        RECT filterCard{adapterCard.right + 10, 140, client.right - 18, 210};
        DrawRoundedPanel(dc, adapterCard, panel, border, 16);
        DrawRoundedPanel(dc, filterCard, panel, border, 16);
        RECT toolbar{18, 222, client.right - 18, 286};
        DrawRoundedPanel(dc, toolbar, panel, border, 16);

        // Accent mark + hero title.
        HBRUSH accent = CreateSolidBrush(s.settings.accent_color);
        RECT accentBar{36, 40, 45, 74};
        FillRect(dc, &accentBar, accent);
        DeleteObject(accent);

        SetBkMode(dc, TRANSPARENT);
        SetTextColor(dc, s.settings.text_color);
        const auto oldFont = SelectObject(dc, s.titleFont ? s.titleFont : s.font);
        RECT title{56, 29, hero.right - 360, 78};
        DrawTextW(dc, L"WireScope", -1, &title, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
        SelectObject(dc, s.smallFont ? s.smallFont : s.font);
        SetTextColor(dc, muted);
        RECT subtitle{56, 75, hero.right - 360, 106};
        DrawTextW(dc, L"Incoming IP monitor", -1, &subtitle, DT_LEFT | DT_VCENTER | DT_SINGLELINE);

        if (s.backgroundArt)
        {
            Graphics g(dc);
            const int w = 300, h = 92;
            const int x = (std::max)(20, static_cast<int>(hero.right) - w - 250);
            ImageAttributes attrs;
            ColorMatrix matrix{};
            matrix.m[0][0] = 1.0f; matrix.m[1][1] = 1.0f; matrix.m[2][2] = 1.0f;
            matrix.m[3][3] = (std::max)(0.0f, (std::min)(0.42f, s.settings.image_opacity / 100.0f));
            matrix.m[4][4] = 1.0f;
            attrs.SetColorMatrix(&matrix);
            g.DrawImage(s.backgroundArt.get(), Rect(x, 24, w, h), 0, 0,
                static_cast<INT>(s.backgroundArt->GetWidth()), static_cast<INT>(s.backgroundArt->GetHeight()), UnitPixel, &attrs);
        }

        // Status and counters on the right.
        const int right = hero.right - 18;
        RECT statusBox{right - 235, 36, right - 142, 103};
        RECT ipsBox{right - 134, 36, right - 68, 103};
        RECT packetsBox{right - 60, 36, right, 103};
        DrawRoundedPanel(dc, statusBox, s.capturing ? Mix(s.settings.background_color, RGB(50,190,115), 0.13) : panel2,
            s.capturing ? RGB(43,125,82) : border, 14);
        DrawRoundedPanel(dc, ipsBox, panel2, border, 14);
        DrawRoundedPanel(dc, packetsBox, panel2, border, 14);

        SelectObject(dc, s.smallFont ? s.smallFont : s.font);
        SetTextColor(dc, muted);
        RECT statusLabel{statusBox.left + 12, statusBox.top + 8, statusBox.right - 8, statusBox.top + 29};
        DrawTextW(dc, L"Status", -1, &statusLabel, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
        RECT ipsLabel{ipsBox.left + 9, ipsBox.top + 8, ipsBox.right - 6, ipsBox.top + 29};
        DrawTextW(dc, L"IPs", -1, &ipsLabel, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
        RECT packetLabel{packetsBox.left + 7, packetsBox.top + 8, packetsBox.right - 4, packetsBox.top + 29};
        DrawTextW(dc, L"Packets", -1, &packetLabel, DT_LEFT | DT_VCENTER | DT_SINGLELINE);

        SelectObject(dc, s.metricFont ? s.metricFont : s.font);
        SetTextColor(dc, s.settings.text_color);
        RECT statusValue{statusBox.left + 12, statusBox.top + 29, statusBox.right - 7, statusBox.bottom - 6};
        DrawTextW(dc, s.status.c_str(), -1, &statusValue, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
        const std::wstring ipCount = std::to_wstring(s.rows.size());
        RECT ipsValue{ipsBox.left + 9, ipsBox.top + 29, ipsBox.right - 6, ipsBox.bottom - 6};
        DrawTextW(dc, ipCount.c_str(), -1, &ipsValue, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
        const std::wstring packetCount = std::to_wstring(s.totalPackets);
        RECT packetValue{packetsBox.left + 7, packetsBox.top + 29, packetsBox.right - 4, packetsBox.bottom - 6};
        DrawTextW(dc, packetCount.c_str(), -1, &packetValue, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);

        // Adapter + filter summary cards.
        SelectObject(dc, s.smallFont ? s.smallFont : s.font);
        SetTextColor(dc, s.settings.accent_color);
        RECT adapterLabel{adapterCard.left + 14, adapterCard.top + 8, adapterCard.right - 10, adapterCard.top + 28};
        DrawTextW(dc, L"Adapter", -1, &adapterLabel, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
        RECT filterLabel{filterCard.left + 14, filterCard.top + 8, filterCard.right - 10, filterCard.top + 28};
        DrawTextW(dc, L"Active filter", -1, &filterLabel, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
        SelectObject(dc, s.font);
        SetTextColor(dc, s.settings.text_color);
        const auto adapterText = CurrentAdapterText(s);
        RECT adapterValue{adapterCard.left + 14, adapterCard.top + 30, adapterCard.right - 12, adapterCard.bottom - 8};
        DrawTextW(dc, adapterText.c_str(), -1, &adapterValue, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
        const auto filterText = CurrentFilterText(s);
        RECT filterValue{filterCard.left + 14, filterCard.top + 30, filterCard.right - 12, filterCard.bottom - 8};
        DrawTextW(dc, filterText.c_str(), -1, &filterValue, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);

        SelectObject(dc, s.smallFont ? s.smallFont : s.font);
        SetTextColor(dc, muted);
        RECT footer{20, client.bottom - 32, client.right - 20, client.bottom - 6};
    ...
    void LayoutMain(AppState& s, int width, int height)
    {
        ...
        const int adapterW = compact ? 205 : (std::min)(360, (std::max)(270, width / 4));
        const int presetW = compact ? 105 : 130;
        const int searchW = compact ? 150 : (std::min)(230, (std::max)(180, width / 7));
        const int buttonW = compact ? 66 : 82;
        const int gap = compact ? 5 : 7;

        MoveWindow(s.adapterCombo, x, y, adapterW, h, TRUE); x += adapterW + gap;
        MoveWindow(s.presetCombo, x, y, presetW, h, TRUE); x += presetW + gap;
        MoveWindow(s.searchEdit, x, y, searchW, h, TRUE); x += searchW + gap;
        ...
        const int listY = 300;
        MoveWindow(s.list, 18, listY, width - 36,
            (std::max)(80, height - listY - 44), TRUE);
    }
    ...
    LRESULT CALLBACK SettingsProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
    {
        ...
        case WM_CTLCOLORLISTBOX:
            SetBkColor(reinterpret_cast<HDC>(wp), Mix(s->settings.background_color, RGB(255,255,255), 0.045));
            SetTextColor(reinterpret_cast<HDC>(wp), s->settings.text_color);
            return reinterpret_cast<LRESULT>(s->editBrush);
        case WM_ERASEBKGND:
        {
    ...
    LRESULT CALLBACK MainProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
    {
        ...
        case WM_CREATE:
        {
            ...
            s->adapterCombo = CreateWindowW(L"COMBOBOX", L"", WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_VSCROLL,
                0,0,100,100, hwnd, reinterpret_cast<HMENU>(IDC_ADAPTER), s->instance, nullptr);
            s->presetCombo = CreateWindowW(L"COMBOBOX", L"", WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_VSCROLL,
            ...
            SetWindowTheme(s->adapterCombo, L"DarkMode_CFD", nullptr);
            SetWindowTheme(s->presetCombo, L"DarkMode_CFD", nullptr);
            s->searchEdit = CreateWindowExW(0, L"EDIT", L"", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL,
            ...
            s->list = CreateWindowExW(0, WC_LISTVIEWW, L"",
        ...
        case WM_COMMAND:
        {
            ...
            else if (id == IDC_ADAPTER && code == CBN_SELCHANGE)
            {
                InvalidateRect(hwnd, nullptr, FALSE);
            }
        ...
        case WM_NOTIFY:
        {
            ...
            if (hdr->hwndFrom == s->list && hdr->code == NM_CUSTOMDRAW)
            {
                ...
                if (cd->nmcd.dwDrawStage == CDDS_ITEMPREPAINT)
                {
                    ...
                    const bool selected = (ListView_GetItemState(s->list, item, LVIS_SELECTED) & LVIS_SELECTED) != 0;
                    const COLORREF even = Mix(s->settings.background_color, RGB(255,255,255), 0.025);
                    const COLORREF odd = Mix(s->settings.background_color, RGB(255,255,255), 0.045);
                    cd->clrText = s->settings.text_color;
                    cd->clrTextBk = selected ? Mix(s->settings.background_color, s->settings.accent_color, 0.22) : ((item % 2) ? odd : even);
                    return CDRF_NEWFONT;
                }
            }

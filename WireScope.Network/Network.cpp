...
namespace
{
    ...
    std::wstring FriendlyNameForPcapId(const char* pcapName)
    {
        if (!pcapName || !*pcapName) return {};
        const std::string wanted = NormalizeGuid(pcapName);
        if (wanted.empty()) return {};

        ULONG size = 0;
        constexpr ULONG flags = GAA_FLAG_SKIP_ANYCAST | GAA_FLAG_SKIP_MULTICAST | GAA_FLAG_SKIP_DNS_SERVER;
        if (GetAdaptersAddresses(AF_UNSPEC, flags, nullptr, nullptr, &size) != ERROR_BUFFER_OVERFLOW || size == 0)
            return {};
        ...
        for (auto* a = addresses; a; a = a->Next)
        {
            ...
            if (NormalizeGuid(a->AdapterName) == wanted && a->FriendlyName && *a->FriendlyName)
    ...
    std::vector<AdapterInternal> EnumerateAdapters()
    {
        ...
        for (auto* dev = devices; dev; dev = dev->next)
        {
            ...
            item.id = dev->name;
            item.name = FriendlyNameForPcapId(dev->name);
            if (item.name.empty()) item.name = ToWide(dev->description);
